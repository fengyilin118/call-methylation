...
void* pthread_event(void* voidargs) {
   
    
    pthread_arg_meth* args = (pthread_arg_meth*)voidargs;
    db_t* db = args->db;
    core_t* core = args->core;
    int batch_id = args->batch_id;
    double realtime0 = core->realtime0;

    fprintf(stderr,"[%.3f] batch %d pthread_event start\n",realtime()-realtime0, batch_id);
   ...
   double event_start = realtime();
    ...
    //create threads
     ...
     //   fprintf(stdout,"n_bam_rec %d num_thread %d\n", db->n_bam_rec, num_thread);
       ...
       fprintf(stderr,"[%.3f] batch %d pthread_event end\n",realtime()-realtime0, batch_id);

    double event_end = realtime();
    core->event_time += (event_end-event_start);

    pthread_mutex_lock(&args->mutex[0]);
    pthread_cond_signal(&args->cond[0]);
    args->finished[0]=1;
    pthread_mutex_unlock(&args->mutex[0]);

    pthread_exit(0);

}

void* pthread_align(void* voidargs)
{
    ...
    pthread_mutex_lock(&args->mutex[0]);
    while(args->finished[0]==0){
        pthread_cond_wait(&args->cond[0], &args->mutex[0]);
    }
    pthread_mutex_unlock(&args->mutex[0]);

    fprintf(stderr,"[%.3f] batch %d pthread_align start\n",realtime()-realtime0, batch_id);
    ...
    fprintf(stderr,"[%.3f] batch %d pthread_align end\n", realtime()-realtime0,batch_id);

    double align_end = realtime();
    core->align_time += (align_end-align_start);

     pthread_mutex_lock(&args->mutex[1]);
     pthread_cond_signal(&args->cond[1]);
    args->finished[1]=1;
    pthread_mutex_unlock(&args->mutex[1]);
...
void* pthread_scaling(void* voidargs)
{
    pthread_arg_meth* args = (pthread_arg_meth*)voidargs;
    db_t* db = args->db;
    core_t* core = args->core;
    ...
    double realtime0 = core->realtime0;
     ...
     //wait
    pthread_mutex_lock(&args->mutex[1]);
    while(args->finished[1]==0){
        pthread_cond_wait(&args->cond[1], &args->mutex[1]);
    }
    pthread_mutex_unlock(&args->mutex[1]);
...
void* pthread_hmm(void* voidargs)
{
    ...
    double realtime0 = core->realtime0;
     //wait
    pthread_mutex_lock(&args->mutex[2]);
    while(args->finished[2]==0){
        pthread_cond_wait(&args->cond[2], &args->mutex[2]);
    }
    pthread_mutex_unlock(&args->mutex[2]); 

    double meth_start = realtime();

   fprintf(stderr,"[%.3f] batch %d pthread_hmm start\n", realtime()-realtime0,batch_id);

    hmm_cuda(core,db,batch_id);

     double meth_end = realtime();
    core->meth_time += (meth_end-meth_start);

    pthread_mutex_lock(&args->mutex[3]);
    pthread_cond_signal(&args->cond[3]);
    args->finished[3]=1;
    pthread_mutex_unlock(&args->mutex[3]);

   

    fprintf(stderr,"[%.3f] batch %d pthread_hmm end\n", realtime()-realtime0, batch_id);
    pthread_exit(0);
...
int align_db(core_t* core, db_t* db, pthread_t tid_align) {
#ifdef HAVE_CUDA
    ...
    if(!db->read_stat_flag[i]){
        if(core->mode==0){
            calculate_methylation_for_read(db->site_score_map[i], db->fasta_cache[i], db->bam_rec[i], db->read_len[i], db->et[i].event, db->base_to_event_map[i],
                    db->scalings[i], core->cpgmodel, core->kmer_size, db->events_per_base[i], i);
        }
        else if (core->mode==1){
...
void process_db(core_t* core, db_t* db, int batch_id) {

//    fprintf(stdout,"process_db batch %d start \n", batch_id);
    ...
    if((core->opt.flag&F5C_SEC_PROF) || (!(core->opt.flag & F5C_DISABLE_CUDA))){
        ...
        double event_start = realtime();
        fprintf(stderr, "Events computed start\n");
        pthread_db(core,db,event_single);
        fprintf(stdout,"Events computed end\n");
        double event_end = realtime();
        core->event_time += (event_end-event_start);

        fprintf(stderr, "[%s::%.3f*%.2f] Events computed\n", __func__, realtime() - realtime0, cputime() / (realtime() - realtime0));
...
void output_db(core_t* core, db_t* db) {
    ...
    for (i = 0; i < core->total_num_reads; i++){
            ...
            if(core->mode==0) {
                std::map<int, ScoredSite> *site_score_map = core->site_score_map[i];
                ...
                for(auto iter = site_score_map->begin(); iter != site_score_map->end(); ++iter) {
                    ...
                    if( !( (core->clip_start != -1 && ss.start_position < core->clip_start) ||
                        ...
                        printf("%d\t%d\t%s\n", ss.strands_scored, ss.n_cpg, ss.sequence);
                    }
