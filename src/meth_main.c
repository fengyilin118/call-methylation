...
void* pthread_processor(void* voidargs) {
    ...
    if(core->opt.verbosity > 1){
       // fprintf(stdout, "[%s::%.3f*%.2f] Signal sent by processor thread!\n", __func__,
       //         realtime() - realtime0, cputime() / (realtime() - realtime0));
    }
...
void* pthread_post_processor(void* voidargs){
  ...
  /*  pthread_arg2_t* args = (pthread_arg2_t*)voidargs;
    ...
    fprintf(stderr,"output and free\n");
    ...
    */
    ...
    db_t* db = pt_arg->db;
    core_t* core = pt_arg->core;
    int batch_id = pt_arg->batch_id;
    double realtime0=core->realtime0;

    //wait until the processing thread has informed us
     //wait
        pthread_mutex_lock(&pt_arg->mutex[3]);
        while(pt_arg->finished[3]==0){
            pthread_cond_wait(&pt_arg->cond[3], &pt_arg->mutex[3]);
        }
        pthread_mutex_unlock(&pt_arg->mutex[3]); 
   ...
   if(batch_id%3==2)
   {
    ...
    fprintf(stderr,"[%.3f] batch %d output and free end \n",realtime() - realtime0,batch_id);
    pthread_exit(0);
...
int meth_main(int argc, char* argv[], int8_t mode) {
    ...
    while ((c = getopt_long(argc, argv, optstring, long_options, &longindex)) >= 0) {
        if (c == 'r') {
           ...
           //   fprintf(stdout,"batch_size_bases %d\n",  opt.batch_size_bases);
    ...
    while (status.num_reads >= core->opt.batch_size || status.num_bases>=core->opt.batch_size_bases) {
     ...
     //   fprintf(stdout, "[%s::%.3f*%.2f] %d Entries (%.1fM bases) loaded\n", __func__,
       //         realtime() - realtime0, cputime() / (realtime() - realtime0),
     ...
     //   fprintf(stdout, "[%s::%.3f*%.2f] %d Entries (%.1fM bases) processed\n", __func__,
      //          realtime() - realtime0, cputime() / (realtime() - realtime0),
       //         status.num_reads,status.num_bases/(1000.0*1000.0));
    ...
    pthread_t tid_event; //event_single thread
    pthread_t tid_align; //align_db thread
    pthread_t tid_scaling;//scaling_single thread
    pthread_t tid_hmm; //hmm score thread
...
/*
    while (status.num_reads >= core->opt.batch_size || status.num_bases>=core->opt.batch_size_bases) {
        ...
        if(batch_id!=55&&batch_id!=184&&batch_id!=185)
        {
...
*/
    ...
    while (status.num_reads >= core->opt.batch_size || status.num_bases>=core->opt.batch_size_bases) {
       ...
       double load_start = realtime();
        ...
        {
            db[id]= init_db(core);
            fprintf(stderr,"[%.3f] load batch %d start\n", realtime() - realtime0, batch_id+id);
             status = load_db(core, db[id]);
             fprintf(stderr, "[%.3f*%.2f] batch %d: %d Entries (%.1fM bases) loaded\n", batch_id+id,
                realtime() - realtime0, cputime() / (realtime() - realtime0),
                status.num_reads,status.num_bases/(1000.0*1000.0));
            total_reads+= status.num_reads;
        }
      ...
      //  if(batch_id==55)|batch_id==188||batch_id==191||batch_id==194||batch_id==197)
        ...
        for(int id = 0;id<3;id++)
        {
            ...
            if(batch_id!=55&&batch_id!=184&&batch_id!=185)
            {
                pthread_arg_meth *pt_arg = (pthread_arg_meth*)malloc(sizeof(pthread_arg_meth));
                pt_arg->core=core;
                pt_arg->db=db[id];
                pt_arg->batch_id = batch_id;
                pt_arg->realtime0 = realtime0;
                for(int i =0; i<4; i++)
                {
                   pt_arg->mutex[i] = PTHREAD_MUTEX_INITIALIZER;
                   pt_arg->cond[i] = PTHREAD_COND_INITIALIZER;;
                   pt_arg->finished[i] = 0;
                }
            ...
            if(batch_id>0)
            {
               int ret = pthread_join(core->tid_event, NULL);
               NEG_CHK(ret);
            }

             int ret = pthread_create(&core->tid_event, NULL, pthread_event,
                                (void*)(pt_arg));
            NEG_CHK(ret);

            if(batch_id>0)
            {
                int ret = pthread_join(core->tid_align, NULL);
                NEG_CHK(ret);
            }
         
            ret = pthread_create(&core->tid_align, NULL, pthread_align ,  (void*)(pt_arg));
            NEG_CHK(ret);

            if(batch_id>0)
            {
                int ret = pthread_join(core->tid_scaling, NULL);
                NEG_CHK(ret);
            }

            ret = pthread_create(&core->tid_scaling, NULL, pthread_scaling ,  (void*)(pt_arg));
             NEG_CHK(ret);

            if(batch_id>0)
           {
                int ret = pthread_join(core->tid_hmm, NULL);
                NEG_CHK(ret);
            }
    ...
    core->process_db_time = total_time - load_time;
    ...
    ret = pthread_join(core->tid_align, NULL);
    NEG_CHK(ret);
 
    ret = pthread_join(core->tid_scaling, NULL);
    NEG_CHK(ret);

    ret = pthread_join(core->tid_hmm, NULL);
     NEG_CHK(ret);
