...
void init_cuda(core_t* core){
    ...
    fprintf(stderr,"init_cuda memory %d\n", n_bam_rec * sizeof(ptr_t)+n_bam_rec * sizeof(int32_t)+n_bam_rec * sizeof(int32_t)+ n_bam_rec * sizeof(ptr_t)+
      n_bam_rec * sizeof(scalings_t)+ MAX_NUM_KMER * sizeof(model_t)+n_bam_rec * sizeof(int32_t) );
...
void launch_align_cuda(core_t* core, db_t* db)
{
 
    int32_t i,j;
    int32_t n_bam_rec = db->n_bam_rec;
    int32_t n_bam_rec_cuda = 0;
    double realtime1;
    //divide into 9 batches
    int64_t batch_bases = core->opt.batch_size_bases/9;

    int64_t batch_ptr[10]; 
    ...
    for (i = 0,j=0; i < n_bam_rec; i++) {
        if(if_on_gpu(core, db, i) && if_gpu_mem_free(core, db, i,sum_read_len,sum_n_events))
        {
            read_ptr_host[j] = sum_read_len;
            sum_read_len += (db->read_len[i] + 1); //with null term
            sum_bases += db->read_len[i];

            sum_n_events += db->et[i].n;
            j++;
            if(sum_bases/batch_bases>bid)
            {
    ...
    // read_len_host = (int32_t*)malloc(sizeof(int32_t) * n_bam_rec);
     //cudaHostRegister(read_len_host,sizeof(int32_t) * n_bam_rec,0);
     cudaMallocHost((void **)&read_len_host, sizeof(int32_t) * n_bam_rec);

    // scalings_host = (scalings_t*)malloc(sizeof(scalings_t) * n_bam_rec);
     //cudaHostRegister(scalings_host,sizeof(scalings_t) * n_bam_rec,0);
     cudaMallocHost((void **)&scalings_host, sizeof(scalings_t) * n_bam_rec);

    // n_event_align_pairs_host = (int32_t*)malloc(sizeof(int32_t) * n_bam_rec);
     //cudaHostRegister(n_event_align_pairs_host, sizeof(int32_t) * n_bam_rec,0);
     cudaMallocHost((void **)&n_event_align_pairs_host, sizeof(int32_t) * n_bam_rec);

   
     
     //form the temporary flattened array on host
     //read_host = (char*)malloc(sizeof(char) * sum_read_len);
     //cudaHostRegister(read_host,sizeof(char) * sum_read_len,0);
     cudaMallocHost((void **)&read_host, sizeof(char) * sum_read_len);
  ...
  //  n_events_host = (int32_t*)malloc(sizeof(int32_t) * n_bam_rec);
   // cudaHostRegister(n_events_host,sizeof(int32_t) * n_bam_rec,0);
   cudaMallocHost((void **)&n_events_host, sizeof(int32_t) * n_bam_rec);

 //   event_ptr_host = (ptr_t*)malloc(sizeof(ptr_t) * n_bam_rec);
  // cudahostregister(event_ptr_host, sizeof(ptr_t) * n_bam_rec,0);
  cudaMallocHost((void **)&event_ptr_host,sizeof(ptr_t) * n_bam_rec);
 ...
 //   cudaHostRegister(event_table_host,sizeof(event_t) * sum_n_events,0);
    cudaMallocHost((void **)&event_table_host,sizeof(event_t) * sum_n_events);
...
//   event_align_pairs_host = (AlignedPair*)malloc(2 * sum_n_events * sizeof(AlignedPair));
 //   cudaHostRegister(event_align_pairs_host, 2 * sum_n_events * sizeof(AlignedPair), 0);
   cudaMallocHost((void **)&event_align_pairs_host, 2 * sum_n_events * sizeof(AlignedPair));

  //  model_t* model_host = (model_t*)malloc(sizeof(model_t) * MAX_NUM_KMER);
  //  cudaHostRegister(model_host, MAX_NUM_KMER * sizeof(model_t), 0);
    model_t* model_host;
    cudaMallocHost((void **)&model_host, sizeof(model_t) * MAX_NUM_KMER);

    for(i=0;i<MAX_NUM_KMER;i++)
      model_host[i] = core->model[i];

    core->align_cuda_preprocess += (realtime() - realtime1);


    int64_t start_id[9], n_reads[9], read_len[9], n_events[9];

    for(int batch_id=0;batch_id<(bid-1);batch_id++)
    {
        start_id[batch_id] = batch_ptr[batch_id];
        n_reads[batch_id] = batch_ptr[batch_id+1] - batch_ptr[batch_id];
        read_len[batch_id] = read_ptr_host[batch_ptr[batch_id+1]] - read_ptr_host[batch_ptr[batch_id]];
        n_events[batch_id] = event_ptr_host[batch_ptr[batch_id+1]] - event_ptr_host[batch_ptr[batch_id]];
    }

    //cudaMalloc() for batch0
    int batch_id = 0;
     /**cuda pointers*/
     char* read0;        //flattened reads sequences
     ptr_t* read_ptr0; //index pointer for flattedned "reads"
     int32_t* read_len0;
     int32_t* n_events0;
     event_t* event_table0;
     ptr_t* event_ptr0;
     scalings_t* scalings0;
     AlignedPair* event_align_pairs0;
     int32_t* n_event_align_pairs0;
     float *bands0;
     uint8_t *trace0;
     EventKmerPair* band_lower_left0;
     model_t* model_kmer_cache0;
     model_t* model0;
 
//   ptr_t* read_ptr_local_host0 = (ptr_t*)malloc(sizeof(ptr_t)*n_reads[0]);
  //  cudaHostRegister(read_ptr_local_host0, sizeof(ptr_t)*n_reads[0],0);
  //  ptr_t* event_ptr_local_host0 = (ptr_t*)malloc(sizeof(ptr_t)*n_reads[0]);
  //  cudaHostRegister(event_ptr_local_host0,sizeof(ptr_t)*n_reads[0],0);
  ptr_t* read_ptr_local_host0;
  ptr_t* event_ptr_local_host0;
  cudaMallocHost((void **)&read_ptr_local_host0, sizeof(ptr_t)*n_reads[0]);
  cudaMallocHost((void **)&event_ptr_local_host0, sizeof(ptr_t)*n_reads[0]);

    for(int64_t id =0; id<n_reads[0]; id++)
    {
        read_ptr_local_host0[id] = read_ptr_host[start_id[0]+id] - read_ptr_host[start_id[0]];
        event_ptr_local_host0[id] = event_ptr_host[start_id[0]+id] - event_ptr_host[start_id[0]];
    }
     ...
     cudaMalloc((void**)&read_ptr0, n_reads[0] * sizeof(ptr_t));
     CUDA_CHK();
     cudaMalloc((void**)&read_len0, n_reads[0] * sizeof(int32_t));
     CUDA_CHK();
     cudaMalloc((void**)&n_events0, n_reads[0] * sizeof(int32_t));
     CUDA_CHK();
     cudaMalloc((void**)&event_ptr0, n_reads[0] * sizeof(ptr_t));
     CUDA_CHK();
     cudaMalloc((void**)&scalings0, n_reads[0] * sizeof(scalings_t));
     CUDA_CHK();
     cudaMalloc((void**)&model0, MAX_NUM_KMER * sizeof(model_t));
     CUDA_CHK();
     cudaMalloc((void**)&n_event_align_pairs0, n_reads[0] * sizeof(int32_t));
     CUDA_CHK();
     cudaMalloc((void**)&read0, read_len[0] * sizeof(char)); //with null char
     CUDA_CHK();
     cudaMalloc((void**)&event_table0, n_events[0] * sizeof(event_t));
     CUDA_CHK();
     cudaMalloc((void**)&model_kmer_cache0, read_len[0] * sizeof(model_t));
 
     // allocate output arrays for cuda
     cudaMalloc((void**)&event_align_pairs0, 2 * n_events[0] * sizeof(AlignedPair)); //todo : need better huristic
     CUDA_CHK();
 
     //scratch arrays
     size_t sum_n_bands = n_events[0] + read_len[0]; //todo : can be optimised
     cudaMalloc((void**)&bands0,sizeof(float) * sum_n_bands * ALN_BANDWIDTH);
     CUDA_CHK();
     cudaMalloc((void**)&trace0, sizeof(uint8_t) * sum_n_bands * ALN_BANDWIDTH);
     cudaMemset(trace0,0,sizeof(uint8_t) * sum_n_bands * ALN_BANDWIDTH); //initialise the trace array to 0
     CUDA_CHK();
     cudaMalloc((void**)&band_lower_left0, sizeof(EventKmerPair)* sum_n_bands);
     CUDA_CHK();
 
     core->align_cuda_malloc += (realtime() - realtime1);


 //cudaMalloc() for batch1
    batch_id = 1;
     /**cuda pointers*/
     char* read1;        //flattened reads sequences
     ptr_t* read_ptr1; //index pointer for flattedned "reads"
     int32_t* read_len1;
     int32_t* n_events1;
     event_t* event_table1;
     ptr_t* event_ptr1;
     scalings_t* scalings1;
     AlignedPair* event_align_pairs1;
     int32_t* n_event_align_pairs1;
     float *bands1;
     uint8_t *trace1;
     EventKmerPair* band_lower_left1;
     model_t* model_kmer_cache1;
     model_t* model1;
 
  //  ptr_t* read_ptr_local_host1 = (ptr_t*)malloc(sizeof(ptr_t)*n_reads[1]);
   // cudaHostRegister(read_ptr_local_host1, sizeof(ptr_t)*n_reads[1], 0);
   // ptr_t* event_ptr_local_host1 = (ptr_t*)malloc(sizeof(ptr_t)*n_reads[1]);
    //cudaHostRegister(event_ptr_local_host1, sizeof(ptr_t)*n_reads[1], 0);
    ptr_t* read_ptr_local_host1;
    ptr_t*  event_ptr_local_host1;
    cudaMallocHost((void **)&read_ptr_local_host1, sizeof(ptr_t)*n_reads[1]);
    cudaMallocHost((void **)&event_ptr_local_host1, sizeof(ptr_t)*n_reads[1]);

    for(int64_t id =0; id<n_reads[1]; id++)
    {
        read_ptr_local_host1[id] = read_ptr_host[start_id[1]+id] - read_ptr_host[start_id[1]];
        event_ptr_local_host1[id] = event_ptr_host[start_id[1]+id] - event_ptr_host[start_id[1]];
    }
     ...
     cudaMalloc((void**)&read_ptr1, n_reads[1] * sizeof(ptr_t));
     CUDA_CHK();
     cudaMalloc((void**)&read_len1, n_reads[1] * sizeof(int32_t));
     CUDA_CHK();
     cudaMalloc((void**)&n_events1, n_reads[1] * sizeof(int32_t));
     CUDA_CHK();
     cudaMalloc((void**)&event_ptr1, n_reads[1] * sizeof(ptr_t));
     CUDA_CHK();
     cudaMalloc((void**)&scalings1, n_reads[1] * sizeof(scalings_t));
     CUDA_CHK();
     cudaMalloc((void**)&model1, MAX_NUM_KMER * sizeof(model_t));
     CUDA_CHK();
     cudaMalloc((void**)&n_event_align_pairs1, n_reads[1] * sizeof(int32_t));
     CUDA_CHK();
     cudaMalloc((void**)&read1, read_len[1] * sizeof(char)); //with null char
     CUDA_CHK();
     cudaMalloc((void**)&event_table1, n_events[1] * sizeof(event_t));
     CUDA_CHK();
     cudaMalloc((void**)&model_kmer_cache1, read_len[1] * sizeof(model_t));
 
     // allocate output arrays for cuda
     cudaMalloc((void**)&event_align_pairs1, 2 * n_events[1] * sizeof(AlignedPair)); //todo : need better huristic
     CUDA_CHK();
 
     //scratch arrays
     sum_n_bands = n_events[1] + read_len[1]; //todo : can be optimised
     cudaMalloc((void**)&bands1,sizeof(float) * sum_n_bands * ALN_BANDWIDTH);
     CUDA_CHK();
     cudaMalloc((void**)&trace1, sizeof(uint8_t) * sum_n_bands * ALN_BANDWIDTH);
     cudaMemset(trace1,0,sizeof(uint8_t) * sum_n_bands * ALN_BANDWIDTH); //initialise the trace array to 0
     CUDA_CHK();
     cudaMalloc((void**)&band_lower_left1, sizeof(EventKmerPair)* sum_n_bands);
     CUDA_CHK();
 
     core->align_cuda_malloc += (realtime() - realtime1);
    

    //cudaMalloc() for batch2
    batch_id = 2;
    /**cuda pointers*/
    char* read2;        //flattened reads sequences
    ptr_t* read_ptr2; //index pointer for flattedned "reads"
    int32_t* read_len2;
    int32_t* n_events2;
    event_t* event_table2;
    ptr_t* event_ptr2;
    scalings_t* scalings2;
    AlignedPair* event_align_pairs2;
    int32_t* n_event_align_pairs2;
    float *bands2;
    uint8_t *trace2;
    EventKmerPair* band_lower_left2;
    model_t* model_kmer_cache2;
    model_t* model2;

   //ptr_t* read_ptr_local_host2 = (ptr_t*)malloc(sizeof(ptr_t)*n_reads[2]);
   //cudaHostRegister(read_ptr_local_host2, sizeof(ptr_t)*n_reads[2],0);
   //ptr_t* event_ptr_local_host2 = (ptr_t*)malloc(sizeof(ptr_t)*n_reads[2]);
   //cudaHostRegister(event_ptr_local_host2, sizeof(ptr_t)*n_reads[2],0);
   ptr_t* read_ptr_local_host2; 
   ptr_t* event_ptr_local_host2;
   cudaMallocHost((void **)&read_ptr_local_host2, sizeof(ptr_t)*n_reads[2]);
   cudaMallocHost((void **)&event_ptr_local_host2, sizeof(ptr_t)*n_reads[2]);

   for(int64_t id =0; id<n_reads[2]; id++)
   {
       read_ptr_local_host2[id] = read_ptr_host[start_id[2]+id] - read_ptr_host[start_id[2]];
       event_ptr_local_host2[id] = event_ptr_host[start_id[2]+id] - event_ptr_host[start_id[2]];
   }
   ...
   realtime1 = realtime();
    ...
    cudaMalloc((void**)&read_ptr2, n_reads[2] * sizeof(ptr_t));
    CUDA_CHK();
    cudaMalloc((void**)&read_len2, n_reads[2] * sizeof(int32_t));
    CUDA_CHK();
    cudaMalloc((void**)&n_events2, n_reads[2] * sizeof(int32_t));
    CUDA_CHK();
    cudaMalloc((void**)&event_ptr2, n_reads[2] * sizeof(ptr_t));
    CUDA_CHK();
    cudaMalloc((void**)&scalings2, n_reads[2] * sizeof(scalings_t));
    CUDA_CHK();
    cudaMalloc((void**)&model2, MAX_NUM_KMER * sizeof(model_t));
    CUDA_CHK();
    cudaMalloc((void**)&n_event_align_pairs2, n_reads[2] * sizeof(int32_t));
    CUDA_CHK();
    cudaMalloc((void**)&read2, read_len[2] * sizeof(char)); //with null char
    CUDA_CHK();
    cudaMalloc((void**)&event_table2, n_events[2] * sizeof(event_t));
    CUDA_CHK();
    cudaMalloc((void**)&model_kmer_cache2,read_len[2] * sizeof(model_t));

    // allocate output arrays for cuda
    cudaMalloc((void**)&event_align_pairs2, 2 * n_events[2] * sizeof(AlignedPair)); //todo : need better huristic
    CUDA_CHK();

    //scratch arrays
    sum_n_bands = n_events[2] + read_len[2]; //todo : can be optimised


    cudaMalloc((void**)&bands2,sizeof(float) * sum_n_bands * ALN_BANDWIDTH);
    CUDA_CHK();
    cudaMalloc((void**)&trace2, sizeof(uint8_t) * sum_n_bands * ALN_BANDWIDTH);
    cudaMemset(trace2,0,sizeof(uint8_t) * sum_n_bands * ALN_BANDWIDTH); //initialise the trace array to 0
    CUDA_CHK();
    cudaMalloc((void**)&band_lower_left2, sizeof(EventKmerPair)* sum_n_bands);
    CUDA_CHK();
    ...
    cudaStream_t *streams = (cudaStream_t *) malloc(9 * sizeof(cudaStream_t));

    for (int batch_id = 0; batch_id < 9; batch_id++)
    {
       cudaStreamCreate(&(streams[batch_id]));
    }
    ...
    //cudaMemcpy
     ...
     realtime1 =  realtime();
        // cuda mem copys
        cudaMemcpyAsync(read_ptr0, read_ptr_local_host0, n_reads[0] * sizeof(ptr_t), cudaMemcpyHostToDevice, streams[0]);
        CUDA_CHK();

        cudaMemcpyAsync(read0, &read_host[read_ptr_host[start_id[0]]], read_len[0] * sizeof(char),cudaMemcpyHostToDevice,streams[0]);
        CUDA_CHK();
        //read length : already linear hence direct copy
        cudaMemcpyAsync(read_len0, &read_len_host[start_id[0]], n_reads[0] * sizeof(int32_t), cudaMemcpyHostToDevice, streams[0]);
        CUDA_CHK();
        cudaMemcpyAsync(n_events0, &n_events_host[start_id[0]], n_reads[0] * sizeof(int32_t), cudaMemcpyHostToDevice, streams[0]);
        CUDA_CHK();
        cudaMemcpyAsync(event_ptr0, event_ptr_local_host0, n_reads[0] * sizeof(ptr_t), cudaMemcpyHostToDevice,streams[0]);
        CUDA_CHK();
        cudaMemcpyAsync(event_table0, &event_table_host[event_ptr_host[start_id[0]]], sizeof(event_t) * n_events[0], cudaMemcpyHostToDevice, streams[0]);
        CUDA_CHK()
       //can be interleaved
       cudaMemcpyAsync(scalings0, scalings_host, sizeof(scalings_t) * n_reads[0],cudaMemcpyHostToDevice,streams[0]);
       CUDA_CHK();
       //model
       cudaMemcpyAsync(model0, model_host, MAX_NUM_KMER * sizeof(model_t), cudaMemcpyHostToDevice, streams[0]);
       CUDA_CHK();

       // cuda mem copys
       cudaMemcpyAsync(read_ptr1, read_ptr_local_host1, n_reads[1] * sizeof(ptr_t), cudaMemcpyHostToDevice, streams[1]);
       CUDA_CHK();
       cudaMemcpyAsync(read1, &read_host[read_ptr_host[start_id[1]]], read_len[1] * sizeof(char),cudaMemcpyHostToDevice,streams[1]);
       CUDA_CHK();
       //read length : already linear hence direct copy
       cudaMemcpyAsync(read_len1, &read_len_host[start_id[1]], n_reads[1] * sizeof(int32_t), cudaMemcpyHostToDevice, streams[1]);
       CUDA_CHK();
       cudaMemcpyAsync(n_events1, &n_events_host[start_id[1]], n_reads[1] * sizeof(int32_t), cudaMemcpyHostToDevice, streams[1]);
       CUDA_CHK();
       cudaMemcpyAsync(event_ptr1, event_ptr_local_host1, n_reads[1] * sizeof(ptr_t), cudaMemcpyHostToDevice,streams[1]);
       CUDA_CHK();
       cudaMemcpyAsync(event_table1, &event_table_host[event_ptr_host[start_id[1]]], sizeof(event_t) * n_events[1],cudaMemcpyHostToDevice, streams[1]);
      CUDA_CHK()
      //can be interleaved
      cudaMemcpyAsync(scalings1, scalings_host, sizeof(scalings_t) * n_reads[1],cudaMemcpyHostToDevice,streams[1]);
      CUDA_CHK();
      //model
      cudaMemcpyAsync(model1, model_host, MAX_NUM_KMER * sizeof(model_t), cudaMemcpyHostToDevice, streams[1]);
      CUDA_CHK();

        // cuda mem copys
        cudaMemcpyAsync(read_ptr2, read_ptr_local_host2, n_reads[2] * sizeof(ptr_t), cudaMemcpyHostToDevice, streams[2]);
        CUDA_CHK();
        cudaMemcpyAsync(read2, &read_host[read_ptr_host[start_id[2]]],read_len[2] * sizeof(char),cudaMemcpyHostToDevice,streams[2]);
        CUDA_CHK();
        //read length : already linear hence direct copy
        cudaMemcpyAsync(read_len2, &read_len_host[start_id[2]], n_reads[2] * sizeof(int32_t), cudaMemcpyHostToDevice, streams[2]);
        CUDA_CHK();
        cudaMemcpyAsync(n_events2, &n_events_host[start_id[2]], n_reads[2] * sizeof(int32_t), cudaMemcpyHostToDevice, streams[2]);
        CUDA_CHK();
        cudaMemcpyAsync(event_ptr2, event_ptr_local_host2, n_reads[2] * sizeof(ptr_t), cudaMemcpyHostToDevice,streams[2]);
        CUDA_CHK();
        cudaMemcpyAsync(event_table2, &event_table_host[event_ptr_host[start_id[2]]], sizeof(event_t) * n_events[2],cudaMemcpyHostToDevice, streams[2]);
        CUDA_CHK()
       //can be interleaved
       cudaMemcpyAsync(scalings2, scalings_host, sizeof(scalings_t) * n_reads[2],cudaMemcpyHostToDevice,streams[2]);
       CUDA_CHK();
       //model
       cudaMemcpyAsync(model2, model_host, MAX_NUM_KMER * sizeof(model_t), cudaMemcpyHostToDevice, streams[2]);
       CUDA_CHK();
      ...
      if(n_reads[0]>0){
        ...
        assert(BLOCK_LEN_BANDWIDTH>=ALN_BANDWIDTH);
            dim3 gridpre(1,(n_reads[0] + BLOCK_LEN_READS - 1) / BLOCK_LEN_READS);
            dim3 blockpre(BLOCK_LEN_BANDWIDTH,BLOCK_LEN_READS);
            if(core->opt.verbosity>1) STDERR("grid %d,%d, block %d,%d",gridpre.x,gridpre.y, blockpre.x,blockpre.y);
            align_kernel_pre_2d<<<gridpre, blockpre,0, streams[0]>>>(read0,
                read_len0, read_ptr0,
                n_events0, event_ptr0, model0, kmer_size, n_reads[0], model_kmer_cache0, bands0,trace0,band_lower_left0);
          //  cudaDeviceSynchronize();
            CUDA_CHK();
            if(core->opt.verbosity>1) 
               fprintf(stderr, "[%s::%.3fsec] align-pre kernel done\n", __func__,  realtime() - realtime1);
        }
      core->align_kernel_time += (realtime() - realtime1);
      core->align_pre_kernel_time += (realtime() - realtime1);
      ...
      if(n_reads[1]>0){
        ...
        assert(BLOCK_LEN_BANDWIDTH>=ALN_BANDWIDTH);
            dim3 gridpre(1,(n_reads[1] + BLOCK_LEN_READS - 1) / BLOCK_LEN_READS);
            dim3 blockpre(BLOCK_LEN_BANDWIDTH,BLOCK_LEN_READS);
            if(core->opt.verbosity>1) STDERR("grid %d,%d, block %d,%d",gridpre.x,gridpre.y, blockpre.x,blockpre.y);
            align_kernel_pre_2d<<<gridpre, blockpre,0, streams[1]>>>(read1,
                read_len1, read_ptr1,
                n_events1, event_ptr1, model1, kmer_size, n_reads[1], model_kmer_cache1, bands1,trace1,band_lower_left1);
          //  cudaDeviceSynchronize();
            CUDA_CHK();
            if(core->opt.verbosity>1) 
               fprintf(stderr, "[%s::%.3fsec] align-pre kernel done\n", __func__,
      ...
      if(n_reads[2]>0){
        ...
        assert(BLOCK_LEN_BANDWIDTH>=ALN_BANDWIDTH);
            dim3 gridpre(1,(n_reads[2] + BLOCK_LEN_READS - 1) / BLOCK_LEN_READS);
            dim3 blockpre(BLOCK_LEN_BANDWIDTH,BLOCK_LEN_READS);
            if(core->opt.verbosity>1) STDERR("grid %d,%d, block %d,%d",gridpre.x,gridpre.y, blockpre.x,blockpre.y);
            align_kernel_pre_2d<<<gridpre, blockpre,0, streams[2]>>>(read2,
                read_len2, read_ptr2,
                n_events2, event_ptr2, model2, kmer_size, n_reads[2], model_kmer_cache2, bands2,trace2,band_lower_left2);
          //  cudaDeviceSynchronize();
            CUDA_CHK();
            if(core->opt.verbosity>1) 
               fprintf(stderr, "[%s::%.3fsec] align-pre kernel done\n", __func__,realtime() - realtime1);
    ...
    //core kernel
      ...
      if(n_reads[0]>0){
            assert(BLOCK_LEN_BANDWIDTH>=ALN_BANDWIDTH);
            dim3 grid1(1,(n_reads[0] + BLOCK_LEN_READS - 1) / BLOCK_LEN_READS);
            dim3 block1(BLOCK_LEN_BANDWIDTH,BLOCK_LEN_READS);
            align_kernel_core_2d_shm<<<grid1, block1,0,streams[0]>>>(read_len0, read_ptr0, event_table0, n_events0,
                    event_ptr0, scalings0, n_reads[0], model_kmer_cache0, kmer_size, bands0,trace0,band_lower_left0 );
        ...
        if(n_reads[1]>0){
          assert(BLOCK_LEN_BANDWIDTH>=ALN_BANDWIDTH);
          dim3 grid1(1,(n_reads[1] + BLOCK_LEN_READS - 1) / BLOCK_LEN_READS);
          dim3 block1(BLOCK_LEN_BANDWIDTH,BLOCK_LEN_READS);
          align_kernel_core_2d_shm<<<grid1, block1,0,streams[1]>>>(read_len1, read_ptr1, event_table1, n_events1,
                  event_ptr1, scalings1, n_reads[1], model_kmer_cache1, kmer_size, bands1,trace1,band_lower_left1 );
       //   cudaDeviceSynchronize();
         CUDA_CHK();
          if(core->opt.verbosity>1) fprintf(stderr, "[%s::%.3fsec] align-core kernel done\n", __func__,
          realtime() - realtime1);
      ...
      if(n_reads[2]>0){
        assert(BLOCK_LEN_BANDWIDTH>=ALN_BANDWIDTH);
        dim3 grid1(1,(n_reads[2] + BLOCK_LEN_READS - 1) / BLOCK_LEN_READS);
        dim3 block1(BLOCK_LEN_BANDWIDTH,BLOCK_LEN_READS);
        align_kernel_core_2d_shm<<<grid1, block1,0,streams[2]>>>(read_len2, read_ptr2, event_table2, n_events2,
                event_ptr2, scalings2, n_reads[2], model_kmer_cache2, kmer_size, bands2,trace2,band_lower_left2 );
    ...
    core->align_core_kernel_time += (realtime() - realtime1);
      ...
      //post kernel
        ...
        if(n_reads[0]>0){
                int32_t BLOCK_LEN = core->opt.cuda_block_size;
                dim3 gridpost((n_reads[0] + BLOCK_LEN - 1) / BLOCK_LEN);
                dim3 blockpost(BLOCK_LEN);
                #ifndef WARP_HACK
                    align_kernel_post<<<gridpost, blockpost,0,streams[0]>>>(event_align_pairs0, n_event_align_pairs0,
                        read_len0, read_ptr0, event_table0, n_events0,
                        event_ptr0,scalings0, n_reads[0], model_kmer_cache0, kmer_size, bands0,trace0,band_lower_left0 );
        
                #else
                    assert(BLOCK_LEN>=32);
                    dim3 grid1post((n_reads[0] + (BLOCK_LEN/32) - 1) / (BLOCK_LEN/32));
                    if(core->opt.verbosity>1) STDERR("grid new %d",grid1post.x);
                    align_kernel_post<<<grid1post, blockpost,0, streams[0]>>>(event_align_pairs0, n_event_align_pairs0,
                        read_len0, read_ptr0, event_table0, n_events0,
                        event_ptr0, scalings0, n_reads[0], model_kmer_cache0, kmer_size, bands0,trace0,band_lower_left0 );
                #endif
             //  cudaDeviceSynchronize();
            ...
            if(n_reads[1]>0){
                int32_t BLOCK_LEN = core->opt.cuda_block_size;
                dim3 gridpost((n_reads[1] + BLOCK_LEN - 1) / BLOCK_LEN);
                dim3 blockpost(BLOCK_LEN);
                #ifndef WARP_HACK
                   align_kernel_post<<<gridpost, blockpost,0,streams[1]>>>(event_align_pairs1, n_event_align_pairs1,
                        read_len1, read_ptr1, event_table1, n_events1,
                        event_ptr1,scalings1, n_reads[1], model_kmer_cache1, kmer_size, bands1,trace1,band_lower_left1 );
        
                #else
                    assert(BLOCK_LEN>=32);
                    dim3 grid1post((n_reads[1] + (BLOCK_LEN/32) - 1) / (BLOCK_LEN/32));
                    if(core->opt.verbosity>1) STDERR("grid new %d",grid1post.x);
                    align_kernel_post<<<grid1post, blockpost,0, streams[1]>>>(event_align_pairs1, n_event_align_pairs1,
                        read_len1, read_ptr1, event_table1, n_events1,
                        event_ptr1, scalings1, n_reads[1], model_kmer_cache1, kmer_size, bands1,trace1,band_lower_left1 );
                #endif
             //  cudaDeviceSynchronize();
            ...
            if(n_reads[2]>0){
                int32_t BLOCK_LEN = core->opt.cuda_block_size;
                dim3 gridpost((n_reads[2] + BLOCK_LEN - 1) / BLOCK_LEN);
                dim3 blockpost(BLOCK_LEN);
                #ifndef WARP_HACK
                    align_kernel_post<<<gridpost, blockpost,0,streams[2]>>>(event_align_pairs2, n_event_align_pairs2,
                        read_len2, read_ptr2, event_table2, n_events2,
                        event_ptr2,scalings2, n_reads[2], model_kmer_cache2, kmer_size, bands2,trace2,band_lower_left2 );
        
                #else
                    assert(BLOCK_LEN>=32);
                    dim3 grid1post((n_reads[2] + (BLOCK_LEN/32) - 1) / (BLOCK_LEN/32));
                    if(core->opt.verbosity>1) STDERR("grid new %d",grid1post.x);
                   align_kernel_post<<<grid1post, blockpost,0, streams[2]>>>(event_align_pairs2, n_event_align_pairs2,
                        read_len2, read_ptr2, event_table2, n_events2,
                        event_ptr2, scalings2, n_reads[2], model_kmer_cache2, kmer_size, bands2,trace2,band_lower_left2 );
                #endif
             //  cudaDeviceSynchronize();
            ...
            // copyback ans
            realtime1 =  realtime();
            cudaMemcpyAsync(&n_event_align_pairs_host[start_id[0]], n_event_align_pairs0,n_reads[0] * sizeof(int32_t), cudaMemcpyDeviceToHost, streams[0]);
          CUDA_CHK();
            cudaMemcpyAsync(&event_align_pairs_host[event_ptr_host[start_id[0]]*2], event_align_pairs0, 2 * n_events[0] * sizeof(AlignedPair), cudaMemcpyDeviceToHost, streams[0]);
        CUDA_CHK();
           
    
            // copyback ans
            realtime1 =  realtime();
            cudaMemcpyAsync(&n_event_align_pairs_host[start_id[1]], n_event_align_pairs1,n_reads[1] * sizeof(int32_t), cudaMemcpyDeviceToHost, streams[1]);
          CUDA_CHK();
            cudaMemcpyAsync(&event_align_pairs_host[event_ptr_host[start_id[1]]*2], event_align_pairs1, 2 * n_events[1] * sizeof(AlignedPair), cudaMemcpyDeviceToHost, streams[1]);
        CUDA_CHK();
    
            cudaMemcpyAsync(&n_event_align_pairs_host[start_id[2]], n_event_align_pairs2,n_reads[2] * sizeof(int32_t), cudaMemcpyDeviceToHost, streams[2]);
          CUDA_CHK();
            cudaMemcpyAsync(&event_align_pairs_host[event_ptr_host[start_id[2]]*2], event_align_pairs2, 2 * n_events[2] * sizeof(AlignedPair), cudaMemcpyDeviceToHost, streams[2]);
        CUDA_CHK();
            core->align_cuda_memcpy += (realtime() - realtime1);
    ...
    for(int batch_id = 0; batch_id < 9; batch_id++)
    {
        cudaStreamSynchronize(streams[batch_id]);
        cudaStreamDestroy(streams[batch_id]);
    }
